...
import { Plus, Trash2, Edit3, X, CheckSquare, Square, Filter, Layers, Copy, Search, Upload, FileQuestion, ChevronsUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const QuestionBank = () => {
  ...
  const [selectedSubjectForPackage, setSelectedSubjectForPackage] = useState('');
  const [subjectPackages, setSubjectPackages] = useState<Record<string, string[]>>({});
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const subjectList = Array.from(new Set(
        data
          .map(q => q.subject || 'Umum')
          .filter((value): value is string => typeof value === 'string' && value.trim() !== '')
      ));
      const uniqueSubjects = ['All', ...subjectList];
      setSubjects(uniqueSubjects);
      if (!selectedSubjectForPackage && subjectList.length > 0) {
        setSelectedSubjectForPackage(subjectList[0]);
      }
      const subjectPackageMap: Record<string, string[]> = {};
      data.forEach(q => {
        const subj = q.subject || 'Umum';
        const pkg = (q.package || 'Default').trim();
        subjectPackageMap[subj] = subjectPackageMap[subj] || [];
        if (pkg && !subjectPackageMap[subj].includes(pkg)) {
          subjectPackageMap[subj].push(pkg);
        }
      });
      setSubjectPackages(subjectPackageMap);
      const uniquePackages = ['All', ...Array.from(new Set(
        data
          .map(q => q.package)
          .filter((value): value is string => typeof value === 'string' && value.trim() !== '')
      ))];
  ...
  const availableSubjectList = subjects.filter(s => s !== 'All');
  ...
  const subjectSelectionOptions = allUniqueSubjects.length > 0 ? allUniqueSubjects : ['Umum'];
  const packagesForCurrentSubject = selectedSubjectForPackage
    ? subjectPackages[selectedSubjectForPackage] || []
    : subjectSelectionOptions.length > 0
      ? subjectPackages[subjectSelectionOptions[0]] || []
      : [];

  const handleCreatePackage = () => {
    const trimmed = newPackageName.trim();
    if (!trimmed) return alert('Package name cannot be empty');
    const targetSubject = selectedSubjectForPackage || subjectSelectionOptions[0] || 'Umum';
    if (!targetSubject || targetSubject === 'All') return alert('Select a valid Mapel before creating a Paket');
    const existingForSubject = subjectPackages[targetSubject] || [];
    if (existingForSubject.includes(trimmed)) return alert('Package already exists for the selected Mapel');

    setTempPackages(prev => [...prev, trimmed]);
    setPackages(prev => Array.from(new Set([...prev, trimmed])));
    setSubjectPackages(prev => ({
      ...prev,
      [targetSubject]: [...new Set([...(prev[targetSubject] || []), trimmed])]
    }));
    setNewPackageName('');
    setFilterPackage(trimmed);
    setSelectedIds([]);
    alert(`Package '${trimmed}' added under Mapel '${targetSubject}'. Select questions and click Move to assign.`);
  };

  const handleCreateSubject = () => {
    const trimmed = newSubjectName.trim();
    if (!trimmed) return alert('Mapel name cannot be empty');
    const existingSubjects = Array.from(new Set([...subjects, ...tempSubjects])).filter(s => s !== 'All');
    if (existingSubjects.includes(trimmed)) return alert('Mapel already exists');

    setTempSubjects(prev => [...prev, trimmed]);
    setSubjects(prev => [...prev, trimmed]);
    setSubjectPackages(prev => ({ ...prev, [trimmed]: prev[trimmed] || [] }));
    setNewSubjectName('');
    setFilterSubject(trimmed);
    setHeaderSubjectFilter(trimmed);
    setSelectedSubjectForPackage(trimmed);
    alert(`Mapel '${trimmed}' added. Use the Paket controls to assign packages.`);
  };
    ...
    <AdminLayout>
      ...
      <div className="card bg-surface p-6">
        ...
        <div className="grid gap-4 mb-6 md:grid-cols-2">
          <section className="card bg-surface/60 border border-border rounded-xl p-4 space-y-3 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-widest text-text-muted">Tambah Mapel</span>
              <span className="text-[10px] font-bold text-primary/70">Atur tema soal</span>
            </div>
            <input
              type="text"
              ...
              title="Tambah Mapel baru untuk bank soal"
            />
            <button
            ...
            <p className="text-[11px] text-text-muted">
              Mapel membantu mengelompokkan paket soal per mata pelajaran agar lebih mudah digunakan nanti.
            </p>
          </section>

          <section className="card bg-surface/60 border border-border rounded-xl p-4 space-y-3 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-widest text-text-muted">Tambah Paket per Mapel</span>
              <span className="text-[10px] font-bold text-primary/70">Pilih Mapel</span>
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-black text-text-muted uppercase tracking-widest">Mapel</label>
              <select
                className="input-field w-full text-sm"
                value={selectedSubjectForPackage || subjectSelectionOptions[0] || ''}
                onChange={e => setSelectedSubjectForPackage(e.target.value)}
                title="Pilih Mapel yang ingin ditambahkan paketnya"
              >
                {subjectSelectionOptions.map(subject => (
                  <option key={subject} value={subject}>{subject}</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-black text-text-muted uppercase tracking-widest">Nama Paket</label>
              <input
                type="text"
                className="input-field w-full text-sm"
                placeholder="Contoh: Paket UTBK"
                value={newPackageName}
                onChange={e => setNewPackageName(e.target.value)}
              />
            </div>
            ...
            <p className="text-[11px] text-text-muted">
              {subjectSelectionOptions.length === 0
                ? 'Tambahkan Mapel terlebih dahulu sebelum membuat paket.'
                : packagesForCurrentSubject.length > 0
                  ? `Paket untuk ${selectedSubjectForPackage || subjectSelectionOptions[0]}: ${packagesForCurrentSubject.join(', ')}`
                  : `Belum ada paket untuk ${selectedSubjectForPackage || subjectSelectionOptions[0]}.`}
            </p>
          </section>
        </div>
